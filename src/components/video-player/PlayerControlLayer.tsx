...
const PlayerControlLayer: React.FC<PlayerControlLayerProps> = ({
	...
	const shouldShowInitialSplash =
		canPlay && !error && !isRemoteActive && !started && paused;

	const activeQualityLabel =
		quality?.height && Number.isFinite(quality.height)
			? `${quality.height}p`
			: null;
			...
			>
				...
				{live && (
					<div className="absolute left-2 top-2 z-10 flex items-center gap-1.5 rounded-sm bg-[#09090B]/90 px-2 py-1 text-[10px] font-bold uppercase tracking-wider">
						...
						/>
						<span className={cn(liveEdge ? "text-green-500" : "text-red-500")}>
							LIVE
						</span>
					</div>
				)}
						...
						>
							{audioOptions.length > 1 ? (
								<label className={VIDSTACK_SELECT_WRAP_CLASS} title="Track âm thanh">
									...
									<select
										...
										onChange={(event) =>
											selectOption(audioOptions, event.target.value)
										}
							...
							<label className={VIDSTACK_SELECT_WRAP_CLASS} title="Chất lượng phát">
								...
								<select
									...
									disabled={
										qualityOptions.disabled || !canSetQuality || !canInteract
									}
									className={cn(VIDSTACK_SELECT_CLASS, "w-full")}
